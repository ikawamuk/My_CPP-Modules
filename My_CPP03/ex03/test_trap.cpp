...
void	diamond_special_ability_test(void)
{
	std::cout << "Diamond special ability tests" << std::endl;
	std::auto_ptr<ClapTrap> trap = generate_new_trap();
	DiamondTrap*	diamond = dynamic_cast<DiamondTrap*>(trap.get());
	if (diamond)
		diamond->whoAmI();
	else
		std::cout << "He is not DiamondTrap." << std::endl;
	return ;
}
