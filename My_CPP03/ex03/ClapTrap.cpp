...
{
	std::cout << BLUE;
	std::cout << "Clap parameterized constructorcalled! (name)";
...
ClapTrap::ClapTrap(const Name& name, const HitPoints& hit_points, const EnergyPoints& energy_points, const AttackDamage& attack_damage)
:name_(name), hit_points_(hit_points), energy_points_(energy_points), attack_damage_(attack_damage)
{
	std::cout << BLUE;
	std::cout << "Clap parameterized constructorcalled! (name, hp, ep, ad)";
	std::cout << RESET << std::endl;
}
